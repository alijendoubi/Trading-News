...
export class EventsService {
     ...
     */
    static async getEvents(limit = 50, offset = 0, country, impact) {
        try {
            // Get events from external API
            const externalEvents = await economicCalendarClient.getUpcomingEvents(undefined, undefined, country, impact);
     ...
     */
    static async getEventsByCountry(country, limit = 50) {
        try {
        ...
        try {
            ...
            for (const event of events) {
                try {
                    ...
                    const existing = await EventModel.findByTitleAndDate(event.title, event.date);
                    if (!existing) {
