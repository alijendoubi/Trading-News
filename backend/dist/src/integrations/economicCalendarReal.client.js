...
export class EconomicCalendarRealClient {
     ...
     */
    async getUpcomingEvents(startDate, endDate, country, impact) {
    ...
    parseImpact(importance) {
        if (!importance)
            return 'Medium';
        const level = importance.toString().toLowerCase();
        if (level.includes('high') || level === '3')
            return 'High';
        if (level.includes('low') || level === '1')
            return 'Low';
        return 'Medium';
    }
    getCountryIds(country) {
...
//# sourceMappingURL=economicCalendarReal.client.js.map