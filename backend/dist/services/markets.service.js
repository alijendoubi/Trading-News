...
export class MarketsService {
    ...
    static async getAssets(limit = 100, offset = 0) {
        try {
            ...
            try {
                ...
                dbAssets.assets = dbAssets.assets.map((asset) => {
                    if (asset.type === 'crypto') {
                        const livePrice = cryptoPrices.find(p => p.symbol.toLowerCase() === asset.symbol.toLowerCase());
                        if (livePrice) {
     ...
     * Get assets by type
     */
    static async getAssetsByType(type, limit = 50, offset = 0) {
