...
export class WatchlistModel {
   ...
   */
  static async addToWatchlist(userId: string, assetId: string): Promise<WatchlistRow> {
    const db = getDb();
    ...
    if (!existing.empty) {
      ...
      return { id: doc.id, ...doc.data() } as WatchlistRow;
    }
    ...
    return createDocument<WatchlistRow>(Collections.USER_WATCHLISTS, watchlistData);
  }

  /**
   * Remove asset from user's watchlist
   */
  static async removeFromWatchlist(userId: string, assetId: string): Promise<boolean> {
    const db = getDb();
    const snapshot = await db.collection(Collections.USER_WATCHLISTS)
      .where('user_id', '==', userId)
      .where('asset_id', '==', assetId)
   ...
   */
  static async isInWatchlist(userId: string, assetId: string): Promise<boolean> {
    const db = getDb();
    const snapshot = await db.collection(Collections.USER_WATCHLISTS)
      .where('user_id', '==', userId)
      .where('asset_id', '==', assetId)
