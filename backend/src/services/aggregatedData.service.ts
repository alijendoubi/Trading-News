...
export class AggregatedDataService {
   ...
   */
  async getStockQuote(symbol: string): Promise<AggregatedQuote | null> {
   ...
   */
  async getCryptoQuote(symbol: string): Promise<AggregatedQuote | null> {
   ...
   */
  async getAggregatedNews(limit: number = 50): Promise<AggregatedNews[]> {
    const allNews: AggregatedNews[] = [];

    // Fetch from all news sources in parallel
    const newsPromises = [
      // Original news client (NewsAPI + RSS)
      newsClient.getLatestNews(limit).then(articles =>
        articles.map(a => ({
          title: a.title,
          url: a.url,
          source: a.source,
      ...
      // Currents API
      currentsClient.getLatestNews(undefined, 'en', 20).then(articles =>
        articles.map(a => ({
          title: a.title,
          url: a.url,
      ...
      // GNews API
      gNewsClient.getTopHeadlines('business', 'en', 20).then(articles =>
        articles.map(a => ({
          title: a.title,
          url: a.url,
      ...
      // Finnhub News
      finnhubNewsClient.getMarketNews('forex').then(articles =>
        articles.map(a => ({
          title: a.headline,
          url: a.url,
          source: a.source,
      ...
      cryptoPanicClient.getTrending().then(posts =>
        posts.map(p => ({
          title: p.title,
          url: p.url,
   ...
   */
  async getCryptoNews(limit: number = 20): Promise<AggregatedNews[]> {
    const cryptoNews: AggregatedNews[] = [];

    // CryptoPanic
    const cryptoPanicNews = await cryptoPanicClient.getTrending();
    cryptoNews.push(
      ...cryptoPanicNews.map(p => ({
        title: p.title,
        url: p.url,
        source: p.source.title,
        publishedAt: p.published_at,
   ...
   */
  async getEconomicIndicators(country: string = 'USA'): Promise<any> {
  ...
  async getTopMovers(): Promise<{
    stocks: { gainers: any[]; losers: any[] };
    crypto: { gainers: any[]; losers: any[] };
  }> {
    const result: any = {
