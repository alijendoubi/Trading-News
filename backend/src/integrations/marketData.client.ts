...
export class MarketDataClient {
   ...
   */
  async getCryptoPrices(symbols: string[] = ['bitcoin', 'ethereum']): Promise<CryptoPrice[]> {
   ...
   */
  async getCryptoPricesFromCryptoCompare(symbols: string[]): Promise<CryptoPrice[]> {
   ...
   */
  async getForexPrice(fromCurrency: string, toCurrency: string = 'USD'): Promise<ForexPrice | null> {
   ...
   */
  async getTrendingCrypto(): Promise<CryptoPrice[]> {
    ...
    try {
      ...
      const ids = trending.map((coin: any) => coin.item.id).join(',');
