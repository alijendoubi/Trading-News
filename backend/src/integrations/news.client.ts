...
export class NewsClient {
  ...
  async getLatestNews(limit: number = 20): Promise<NewsArticle[]> {
    ...
    try {
      ...
      const newsPromises = this.rssFeedSources.map(source =>
        this.fetchFromRSS(source.url, source.name, source.category).catch(err => {
  ...
  private async fetchFromRSS(
    ...
    try {
      ...
      for (const match of itemMatches) {
        ...
        if (articles.length >= 10) break; // Limit per source
      }
   ...
   */
  private async fetchFromNewsAPI(limit: number = 20): Promise<NewsArticle[]> {
    try {
