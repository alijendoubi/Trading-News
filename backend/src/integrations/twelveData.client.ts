...
export class TwelveDataClient {
   ...
   */
  async getQuotes(symbols: string[]): Promise<TwelveDataQuote[]> {
  ...
  async getTimeSeries(
    ...
    try {
      ...
      const series: TwelveDataTimeSeries[] = response.data.values.map((item: any) => ({
   ...
   * Get forex pair rate
   */
  async getForexRate(symbol1: string, symbol2: string): Promise<number | null> {
